...
numbers, positions_ang = read_xyz('AQ-geometry.xyz')
print(positions_ang)
...
with open('basis-AQ-gfn1.txt', 'w') as f:
...
df.to_csv('AQ-gfn1_hamiltonian_matrix_xtb.csv', index=False)

do = pd.DataFrame(res.get("overlap-matrix"))
do.to_csv('AQ-gfn1_overlap_matrix_xtb.csv', index=False)

de =27.2114 * pd.DataFrame(res.get("orbital-energies"))
de.to_csv('AQ-gfn1_orbital-energies_xtb.csv', index=False)

dp = pd.DataFrame(res.get("orbital-occupations"))
dp.to_csv('AQ-gfn1_orbital-occupations_xtb.csv', index=False)

dc = pd.DataFrame(res.get("orbital-coefficients"))
dc.to_csv('AQ-gfn1_orbital-coefficients_xtb.csv', index=False)

...
% 从.dat文件中读取数据
h_mm_data = load('pyridine-hamiltonian.dat');
s_mm_data = load('pyridine-overlap.dat');
...
Sigma_3layer_L = diag([Sigma_layer,Sigma_layer,Sigma_layer]);
Sigma_3layer_R = diag([Sigma_layer,Sigma_layer,Sigma_layer]);
...
Sigma_dft = diag([zeros(1,228),1,1,zeros(1,Nm-230)]);
...
for i=1:length(Erange)       
    ...
    G = inv((Erange(i) + z_plus) * S_mol - H_mol - Sigma_L - Sigma_R);   
